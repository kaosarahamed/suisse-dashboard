...
} from "../../../redux/rtk/features/news/newsApi";
import EditNews from "./Edit-news";
import NewsLoading from "../../../loading/News-loading";
...
const News = () => {
  ...
  const { data, isLoading, isError } = useGetAllNewsByAdminQuery({
    page,
    limit,
  ...
  const totalItems = data?.totalNews;
  ...
  if (!isLoading && !isError && data?.news?.length > 0) {
    content = data?.news?.map((item, index) => {
      const {
        nameOfStation,
        senderEmail,
        reference,
      ...
      return (
        <tr key={_id}>
          <td>{index + 1}</td>
          <td>{nameOfStation}</td>
          <td>{senderEmail?.slice(0, 20)}...</td>
          <td>{sendName}</td>
          <td>{reference}</td>
          <td>{news.slice(0, 30)}...</td>
          ...
          <td>
            <div className="action-buttons d-flex justify-content-end">
              <Link to={`/news/${_id}`} className="btn btn-success light mr-2">
                <svg
  ...
  return (
    <div className="custom-position">
      <div className="d-flex align-items-center mb-4 flex-wrap ">
        <h4 className="fs-20 font-w600  me-auto">All News</h4>
        <Tab.Container
      ...
      <div className="row ">
        ...
        <div className="d-md-flex d-block align-items-center justify-content-between text-center  flex-wrap mt-md-0 mt-3">
          <div className="mb-md-0 mb-2">
            <h5 className="mb-0">Showing {totalItems} News</h5>
          </div>
          {totalItems > 0 && (
            <Pagination
          ...
          <EditNews isShow={isShow} setIsShow={setIsShow} id={id} />
        </div>
      </div>
      <Toaster />
    </div>
