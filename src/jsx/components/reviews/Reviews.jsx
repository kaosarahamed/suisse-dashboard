...
import ReviewLoading from "../../../loading/Review-loading";
...
const Reviews = () => {
  ...
  const totalItems = data?.totalreviews;
  ...
  if (!isLoading && !isError && data?.reviews?.length === 0) {
    content = (
      <p style={{ padding: "20px", fontSize: "20px" }}>Reviews not found!</p>
    );
  }
  if (!isLoading && !isError && data?.reviews?.length > 0) {
    content = data?.reviews?.map((item) => {
      const {
        clinetName,
        review,
        rating,
      ...
      return (
        <tr key={_id}>
          <td>{jobTitle}</td>
          <td>{clinetName?.slice(0, 20)}...</td>
          <td className="wspace-no">
            {sellerName ? sellerName : "Not Available"}
          </td>

          <td>{review?.slice(0, 30)}...</td>
          <td>{rating}</td>
          <td>{useful?.length}</td>
  ...
  return (
    <div className="custom-position">
      <div className="d-flex align-items-center mb-4 flex-wrap ">
        <h4 className="fs-20 font-w600  me-auto">All Reviews</h4>
        <Tab.Container
      ...
      <div className="row ">
        ...
        <div className="d-md-flex d-block align-items-center justify-content-between text-center  flex-wrap mt-md-0 mt-3">
          <div className="mb-md-0 mb-2">
            <h5 className="mb-0">Showing {totalItems} Reviews</h5>
          </div>
          {totalItems > 0 && (
            <Pagination
