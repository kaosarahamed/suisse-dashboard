...
class NewCustomers1 extends Component {
  constructor(props) {
    super(props);
  ...
  updateChartData = () => {
    const { jobs } = this.props; // Assuming jobs are passed as props
    ...
    const jobsByDay = last7Days?.map((day) => {
      const jobsOnDay = jobs?.filter((job) => {
        const jobDate = format(new Date(job.createdAt), "dd MMM"); // Compare with formatted date
        return jobDate === day;
      });
      return jobsOnDay?.length > 0 ? jobsOnDay.length : 0;
    });
    ...
    this.setState({
      series: [
        {
          name: "Jobs Posted",
          data: jobsByDay, // Number of jobs posted per day
        },
      ],
      options: {
        ...this.state.options,
