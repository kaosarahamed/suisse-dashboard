...
import CreditLoading from "../../../loading/Credit-loading";
...
const Credit = () => {
  ...
  const { data, isLoading, isError } = useGetAllCreditsByAdminQuery({
    page,
    limit,
  ...
  const totalItems = data?.totalCredits;
  ...
  if (!isLoading && !isError && data?.credits?.length === 0) {
    content = (
      <p style={{ padding: "20px", fontSize: "20px" }}>
        Credit Plans not found!
      </p>
    );
  }
  if (!isLoading && !isError && data?.credits?.length > 0) {
    content = data?.credits?.map((item, index) => {
      const { title, price, credits, createdAt, _id, status } = item || {};
  ...
  return (
    <div className="custom-position">
      <div className="d-flex align-items-center mb-4 flex-wrap ">
        <h4 className="fs-20 font-w600  me-auto">All Credit Plans</h4>
        <Tab.Container
      ...
      <div className="row ">
        ...
        <div className="d-md-flex d-block align-items-center justify-content-between text-center  flex-wrap mt-md-0 mt-3">
          <div className="mb-md-0 mb-2">
            <h5 className="mb-0">Showing {totalItems} Credits</h5>
          </div>
          {totalItems > 0 && (
            <Pagination
          ...
          <EditCredit isShow={isShow} setIsShow={setIsShow} id={id} />
        </div>
      </div>
      <Toaster />
    </div>
